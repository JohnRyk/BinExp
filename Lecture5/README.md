...
​	上面的命令会把格式化字符串的长度 len(AAAA %p %p %p %p %p %p %p %p %p ) --> 32 写入到第 10th argument 所指向的内存地址中. 在这里，由于第10个参数对应的内容是 0x41414141 所以它会尝试把 32 写入0x41414141 这个地址 。 由于这个地址（0x41414141）本身应该是 read-only memory, 所以你会得到一个 `Segmentation Fault`. 
